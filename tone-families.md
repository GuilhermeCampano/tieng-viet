# Tiếng Việt — Tone Families

One `##` section per family, table inside. The same syllable with a different tone is a different word.

---

Frequency ratings: **common** / **uncommon** / **rare**, based on corpus frequency (e.g. Subtlex-VN).

## đắp / đáp / đạp / đập

| Word | Tone | Meaning | Example | Frequency |
|------|------|---------|---------|-----------|
| đắp  | sắc (short ă) | to cover, to build up (wall), to apply (plaster) | Em đắp chăn khi lạnh. — I cover myself with a blanket when cold. | common |
| đáp  | sắc (long a)  | to land; to answer; to board (transport) — Hán-Việt 答 | Máy bay đáp xuống sân bay. — The plane landed at the airport. | common |
| đạp  | nặng          | to pedal, to kick, to tread | Tôi đạp xe đi làm. — I ride my bike to work. | common |
| đập  | nặng (âp)     | to hit, to beat, to slam shut | Đừng đập cửa! — Don't bang the door! | common |

## đặt / đắt / đất / đạt

| Word | Tone | Meaning | Example | Frequency |
|------|------|---------|---------|-----------|
| đặt  | nặng (đăt) | to place, put; to book, order | Đặt gối sau lưng. — Put the pillow behind your back. | common |
| đắt  | sắc (đăt) | expensive | Cái này đắt quá! — This is so expensive! | common |
| đất  | sắc (đât, â) | land, soil, earth | đất nước — country | common |
| đạt  | nặng (đat, long) | to reach, achieve — Hán-Việt 達 | đạt được — to achieve | common |

## la / là / lá / lả / lã / lạ

| Word | Tone | Meaning | Example | Frequency |
|------|------|---------|---------|-----------|
| la   | ngang       | to shout, yell, scold | Mẹ la con vì không học bài. — Mom scolds the kid for not studying. | common |
| là   | huyền       | to be (copula) | Tôi là người Brasil. — I'm Brazilian. | common |
| lá   | sắc         | leaf; sheet (of paper) | Lá cây màu xanh. — The leaf is green. | common |
| lả   | hỏi         | tired, weak, limp | Chạy xong mệt lả người. — Exhausted after running. | uncommon |
| lã   | ngã         | fine silk, gauze; interest (finance) | Lụa là / lãi suất — silk / interest rate | uncommon |
| lạ   | nặng        | strange, unfamiliar, unusual | Món này lạ quá! — This dish is so unusual! | common |

## cần / cấn / cản / căn

| Word | Tone | Meaning | Example | Frequency |
|------|------|---------|---------|-----------|
| cần  | huyền        | to need; (cần câu — fishing rod) | Tôi cần rửa chén. — I need to wash the dishes. | common |
| cấn  | sắc (â)      | to be wedged/trapped; to bump against | Cái gì đó cấn ở lưng tôi. — Something's pressing on my back. | uncommon |
| cản  | hỏi          | to block, to obstruct | Xin đừng cản đường! — Please don't block the way! | uncommon |
| căn  | ngang (â)    | root (Hán-Việt 根); room | căn phòng — room | common |
